...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
       ...
       # You can now check if truth is in classifier label and extend the dictionary accordingly. 
       ...
       # defines truth as pet image label # This is the real label you stored in the dictionary in the previous file
       ...
       # results_dic as an exact match(1) using extend list function
        #match = results_dic[key][1]
        #results_dic[key].extend([match])
...
# Populates dictionary with whether or not labels indicate a dog image (idx 3&4)
        #for key in range (0, len(results_dic[key]), 1):
    # Key already exists, extend values to end of list for idx 3 & 4
            #results_dic[key].extend([pet_label_is_dog[1], classifier_label_is_dog[1]])
